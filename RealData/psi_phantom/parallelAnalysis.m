...
stdEFF=double(std(flatFields,0,2));
keepTrack=double(zeros(size(flatFields,2),repetitions));
stdMatrix=double(repmat(stdEFF,[1,size(flatFields,2)]));
...
mean_flat_fields_EFF=double(mean(flatFields,2));
F=double(flatFields-repmat(mean_flat_fields_EFF,[1,size(flatFields,2)]));
[V1, D1] = eig(cov(F));
D1=diag(D1);
